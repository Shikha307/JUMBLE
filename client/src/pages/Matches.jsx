...
import { SWIPE_API, USER_JOB_API } from '../config/api';
...
function CandidateModal({ match, onClose }) {
    ...
    if (!details) {
      const fetchDetails = async () => {
        try {
          ...
          const res = await fetch(`${USER_JOB_API}/api/candidates/${match.candidateId}`, { headers });
          if (res.ok) {
  ...
  const handleViewResume = async (e, resumeId = null) => {
    ...
    try {
      ...
      const res = await fetch(`${USER_JOB_API}/api/candidates/${match.candidateId}/resume${queryParams}`, {
                  ...
                  {match.allMatchedJobs.map((job, idx) => (
                    <div key={idx} style={{ padding: '1.25rem', background: '#f8fafc', borderRadius: '16px', border: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                        ...
                        {typeof job.matchCoverLetter === 'string' && job.matchCoverLetter.trim().length > 0 && (
                          <button
                            onClick={() => setCoverLetterText(job.matchCoverLetter.trim())}
                            className="nav-action-btn subtle"
                            style={{ padding: '0.6rem 1rem', fontSize: '0.85rem', cursor: 'pointer', border: 'none', background: '#ecfeff', color: '#0f766e', borderRadius: '12px', display: 'flex', alignItems: 'center', gap: '0.5rem', whiteSpace: 'nowrap' }}
                          >
      ...
      {coverLetterText && (
        ...
        >
          ...
          >
            ...
            <h3 style={{ marginTop: 0, color: '#0f172a' }}>Cover Letter</h3>
...
function JobModal({ match, onClose }) {
    ...
    if (!details) {
      const fetchDetails = async () => {
        try {
          const token = localStorage.getItem('token');
          const headers = token ? { 'Authorization': `Bearer ${token}` } : {};
          const res = await fetch(`${USER_JOB_API}/api/jobs/${match.jobId}`, { headers });
          if (res.ok) {
            const data = await res.json();
            setDetails(data);
          } else {
  ...
  useEffect(() => {
    if (details && details.recruiterId) {
      const token = localStorage.getItem('token');
      fetch(`${USER_JOB_API}/api/recruiters/${details.recruiterId}`, {
        headers: token ? { 'Authorization': `Bearer ${token}` } : {}
      })
  ...
  useEffect(() => {
    const fetchMatches = async () => {
      try {
        ...
        let endpoint = '';
        if (role === 'recruiter') {
          endpoint = `${SWIPE_API}/api/v1/matches/recruiter/${id}`;
        } else {
          endpoint = `${SWIPE_API}/api/v1/matches/candidate/${id}`;
        }
        ...
        if (response.ok) {
          ...
          const hydratedMatches = await Promise.all(
            rawMatches.map(async (match) => {
              ...
              if (role === 'recruiter') {
                try {
                  const candRes = await fetch(`${USER_JOB_API}/api/candidates/${match.candidateId}`, { headers: authenticatedHeaders });
              ...
              try {
                const jobRes = await fetch(`${USER_JOB_API}/api/jobs/${match.jobId}`, { headers: authenticatedHeaders });
          ...
          if (role === 'recruiter') {
            const grouped = hydratedMatches.reduce((acc, current) => {
              ...
              if (!acc[key]) {
                acc[key] = {
                  ...current,
                  allMatchedJobs: current.jobDetails ? [{ ...current.jobDetails, matchResumeId: current.candidateResumeId, matchCoverLetter: current.candidateCoverLetterText }] : []
                };
              } else if (current.jobDetails) {
                acc[key].allMatchedJobs.push({ ...current.jobDetails, matchResumeId: current.candidateResumeId, matchCoverLetter: current.candidateCoverLetterText });
              }
              return acc;
            }, {});
